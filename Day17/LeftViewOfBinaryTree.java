...
Time complexity : O(N) which is equal to space complexity
...
class Tree
{
    ...
    public void lv(Node root, int level, ArrayList<Integer> al){
        if(root == null) return ;
...
Time complexity O(N) and has the stack space of O(Height of the tree)